...
the BASE alone (`robots/fetch/extand.py:follow_arc`). So across the pull the gripper's world
...
def side_grasp_pose(task, depth: float, lift: float = 0.0, obj=None):
    """Build side-grasp and pre-grasp poses for the current object pose."""
    ...
    obj_p = _np(obj.pose.p).reshape(-1, 3)[0].astype(np.float64)
    centre = np.array([
        obj_p[0], obj_p[1] + float(depth),
...
# Straight retrieval settings shared by DepthRecall's extraction/restoration flow.
...
def plan_joints(env, planner, task, targets: dict, *, label: str, tries: int = 2,
    ...
    return common.plan_joints(
        env, planner, task, targets, label=label, tries=tries, who=WHO,
        line_only=line_only, qpos_step=qpos_step,
    )
...
def stow_hand(env, planner, task, z: float = 1.10, aheads=(0.70, 0.55, 0.85)) -> None:
    ...
    A local stow sequence keeps SameDrawer-specific planning knobs out of this
    oracle. Why here: after the pull the arm is stretched to the swung bar at
...
def fold_arm_to_rest(env, planner, task, *, label: str = "fold to rest"):
    ...
    return common.plan_joints(
        env, planner, task, targets, label=label, tries=3,
        who=WHO, trace_rrt=False,
    )
...
def close_the_door(env, planner, task, *, door: DoorSpec | None = None, anchor=None,
    ...
    # the twist — the documented BASE_ONLY_PLAN_MASK disease, see robots/fetch/extand.py).
...
def ready_by_line(env, planner, task, ready, *, drive: float, grasp,
                  label: str = "ready posture", slack=None,
                  touch_needle: str | None = "cup", rank=None):
    """Choose an IK goal whose arm line, base drive, and grasp screw all plan."""
    import mplib
    ...
    held = [True, True, True, True] + [False] * (len(cur) - 4)
    try:
    ...
    except Exception as exc:  # solver refusal, not an episode error
        status, goals = f"IK raised {type(exc).__name__}: {exc}", None
    ...
    for k, g in enumerate(order[:READY_IK_GOALS]):
        ...
        if float(drive):
            ...
            try:
                ...
                sweep = {"status": f"mplib raised: {exc}"}
            if sweep.get("status") != "Success":
                tried.append(f"g{k}: drive {sweep.get('status')}")
                continue
        off = (0.0, READY_GRASP_SLACK, 0.0) if slack is None else slack
        deeper = mplib.Pose(p=np.asarray(grasp.p) + np.asarray(off), q=grasp.q)
        try:
            with (common.touchable(planner, touch_needle) if touch_needle
                  else contextlib.nullcontext()):
                screw = p.plan_screw(
                    deeper, driven, time_step=dt, masked_joints=~np.array(held),
                    goal_tolerance=planner.ARM_SCREW_GOAL_TOLERANCE,
                )
        except Exception as exc:
            screw = {"status": f"screw raised {type(exc).__name__}"}
        if screw.get("status") != "Success":
            tried.append(f"g{k}: grasp {screw.get('status')}")
            continue
        say(env, f"{label}: joint line", knots=len(np.asarray(line["position"])),
            ik_goal=k, ik_goals=len(order))
...
def solve(env, seed=None, debug=False, vis=False, blind=False,
    ...
    res = common.plan_joints(
        env, planner, task,
        {"torso_lift_joint": TORSO_DRIVE, "wrist_flex_joint": 1.7},
        label="duck the torso", who=WHO, trace_rrt=False,
    )
    if res != -1 and common.stopped_by_horizon(planner):
        return res
    if res == -1:
    ...
    if float(task.cfg.door_open_rad) < ARM_PASS_RAD:
        ...
        res = common.plan_joints(
            env, planner, task,
            {"torso_lift_joint": TORSO_DRIVE, "wrist_flex_joint": 1.7},
            label="re-duck after the door", who=WHO, trace_rrt=False,
        )
        if res != -1 and common.stopped_by_horizon(planner):
            return res
        if res == -1:
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    env = gym.make("MikasaCabinetRetrieval-v0", num_envs=1, robot_uids="ds_fetch",
