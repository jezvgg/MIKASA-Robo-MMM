"""Run a scripted planner over many episodes and report its success rate.
    ...
    python -m utils.test_planner -n 100 --scene-idx 0
    python -m utils.test_planner -n 25 --start-seed 102 --seed-step 0
...
## `--blind`: the memoryless control arm, over the same sweep
    ...
    python -m utils.test_planner -s MikasaBurner-v0 -p burner_planner \
...
## `--info-keys`: per-key tallies over the sweep
    ...
    python -m utils.test_planner -s MikasaStationChecklist-v0 \
...
## `--log-dir`: a text trace per episode
    ...
    python -m utils.test_planner -p season_dish_planner -n 5 \
        --log-dir logs/season --obs-mode state

Each episode then writes `<log-dir>/seed_<seed>/` — an events `.jsonl` and one
`_trajectory.csv` per tracked object. `utils.logging_utils` documents how to
...
def classify_result(result) -> str:
    ...
    all sweep buckets are derived from it. The precedence is
...
def parse_args(argv=None):
    ...
    p.add_argument(
        ...
        "for what it costs and utils.logging_utils for how to read it",
    )
    p.add_argument(
        "--log-freq",
        type=int,
...
def run_sweep(args, *, make_env=gym.make, recorder_cls=RecordEpisode) -> SweepResult:
    ...
    for line in _format_info_keys(per_key, info_keys):
        print(line)
    if log_root is not None:
        print(f"traces in {log_root}/ — see utils.logging_utils for how to read them")
