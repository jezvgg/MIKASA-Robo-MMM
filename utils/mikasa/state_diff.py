...
def snapshot(env, info=None, objects=None) -> dict:
    ...
    Example:
        ...
        >>> env = gym.make("MyRoboCasa-v1", num_envs=1, robot_uids="ds_fetch",
...
def build_env(task: str, scene_idx=0, render_size: int = 256):
    ...
    would reject the kwarg (`utils.test_planner.accepts_scene_idx`).
    ...
    from utils.test_planner import accepts_scene_idx
...
def run(task: str, scene_idx=0, seed: int = 3, at=(0, 10),
    ...
    print(
        f"=== state diff: {task} scene_idx={scene_idx} seed={seed} "
        f"at={targets} ===",
        flush=True,
    )
    snapshots = []
...
def parse_args(argv=None):
    ...
    Returns:
        `argparse.Namespace` with `task, scene_idx, seed, at, frames,
    ...
    Example:
        ...
        >>> args.task, args.at, args.scene_idx
        ('MikasaSeasonDish-v0', '0,30,60', 0)
    """
